...
C
      WRITE(*,10001)NK,NSUM,NNANS,AMIN,AMAX,ARMS,AVE
10001 FORMAT('no. of entries = ',I6,'  no. non-zero = ',I6,
     :       '  no.of.NaNs = ',I6,/,'min = ',G12.5,'  max = ',G12.5,
     :       '  rms = ',G12.5,'  average = ',G12.5)
      RETURN
      END

      SUBROUTINE DMATPP(ARR,NK)
C
...
C
      WRITE(*,10001)NK,NSUM,NNANS,AMIN,AMAX,ARMS,AVE
10001 FORMAT('no. of entries = ',I6,'  no. non-zero = ',I6,
     :       '  no.of.NaNs = ',I6,/,'min = ',G14.5,'  max = ',G14.5,
     :       '  rms = ',G14.5,'  average = ',G14.5)
      RETURN
      END

      SUBROUTINE IMATPP(IARR,NK)
C
...
C
      DO J=1,NK
C       IF(ISNAN(IARR(J)))THEN    ! ISNAN not defined for integers
C       ENDIF
        IF(IARR(J).EQ.0)THEN
          NZERO=NZERO+1
        ELSE
          ...
          NSUM=NSUM+1
        ENDIF
        IF(IARR(J).GT.IMAX)THEN
          IMAX=IARR(J)
          JMAX=J
        ELSEIF(IARR(J).LT.IMIN)THEN
          IMIN=IARR(J)
          JMIN=J
        ENDIF
      ENDDO
...
C    report array facts
C
      WRITE(*,10001)NK,NSUM,NZERO,IMIN,IMAX,ARMS,AVE
10001 FORMAT('no. of entries = ',I6,'  no. non-zero = ',I6,
     :       '  no. zeros = ',I6,/,'min = ',I8,'  max = ',I8,
     :       '  rms = ',G12.5,'  average = ',G12.5)
      RETURN
      END
