...
C10116 FORMAT(2I6,2(F13.8,F11.6))
C10117 FORMAT(2I6,2(F13.8,F11.6),F13.5,F11.4)
C10118 FORMAT(2I6,4(F13.8,F11.6))
10116 FORMAT(2I6,4G13.5)
10117 FORMAT(2I6,6G13.5)
10118 FORMAT(2I6,8G13.5)
      END
C
C----------------------------------------------------------------
C
     ...
     :                 VISCMIN,VISCMAX,ED2IMIN,ED2IMAX,THDMIN,
     :                 THDIMAX,POLEP,NE,NBP,NN,NUP,NFP,NROWS,IREAD,
C    :                 KBWP,K2BWP,K4BWP,NKP,NK2P,NK4P,IREAD,
     :                 IREM,IT,RMSU,RMSV,NITER,IOFF,NFPF3,IVERB,IERR)
C
...
C
      ...
      COMMON/STKDIM/KBWP,K2BWP,NKP,NK2P,K4BWP,NK4P
      SAVE /STKDIM/
      ...
      DIMENSION NOR(NUP)
      DIMENSION SSQ(NUP)
      DIMENSION XLOAD(NROWS),PDP(NROWS),XWRK(NROWS)           ! local
      ...
      DIMENSION INTV(64)
      DIMENSION RLV(64)
      DIMENSION STK(NKP),STK2(NK2P),STK3(NFP),STK4(NK4P)      ! local
...
C
      ...
      TREFD=RLV(ITREF)
      CENTLNG=0.0
      IF(INTV(IIDEFTYP).EQ.110)CENTLNG=RLV(IXREFM)
...
C
      DO J=1,NROWS
        XWRK(J)=UVP(J)
      ENDDO
C     WRITE(*,*)'XWRK before iterations'
C     CALL DMATPP(VHB,NROWS)
C     WRITE(*,*)'STK4 before iterations'
C     CALL DMATPP(STK4,NK4P)
C     WRITE(*,*)'VHB before SEMBL'
C     CALL SMATPP(VHB,8*NE)
C     WRITE(*,*)'QBND before SEMBL'
C     CALL SMATPP(QBND,2*NBP)
C     WRITE(*,*)'IBCTYP before SEMBL'
C     CALL IMATPP(IBCTYP,2*NBP)
C
...
C
      CALL LOADBC(BIG,XLOAD,QLOAD,QBND,EX,EY,IBC,IBCTYP,
     :            POLEP,MAXPOLE,CENTLNG,NUP,NBP,NROWS,INTV(IIPOLE))
C
...
C
     ...
     :               NN,NFP,RLV(ISE))
      ENDIF
C     WRITE(*,*)'Pressure elements:'
C     CALL DMATPP(XWRK(2*NUP+1),NN)
C
...
C
      DO 80 II=1,NBP
        ...
        NSY=(NUP+NR-1)*KBW+1
        IY=II+NBP
C       IF(IBCTYP(II).EQ.0.OR.IBCTYP(II).EQ.10)STK(NSX)=BIG
C       IF(IBCTYP(IY).EQ.0.OR.IBCTYP(IY).EQ.10)STK(NSY)=BIG
        IF(IBCTYP(II).EQ.0.OR.IBCTYP(II).GT.200)STK(NSX)=BIG
        IF(IBCTYP(IY).EQ.0.OR.IBCTYP(IY).GT.200)STK(NSY)=BIG
...
C
  ...
  230 CONTINUE              !  on I
      ...
      END
C
      SUBROUTINE LOADBC(BIG,XLOAD,QLOAD,QBND,EX,EY,IBC,IBCTYP,
     :                  POLEP,MAXPOLE,CENTLNG,NUP,NBP,NROWS,IPOLE)
C
...
C
        IF(ITYPX.EQ.0)XLOAD(NNX)=BIG*QBND(I)
        IF(ITYPY.EQ.0)XLOAD(NNY)=BIG*QBND(I+NBP)
C
...
C       WRITE(*,*)I,QBND(I),QBND(I+NBP),ITYPX,ITYPY
  200 CONTINUE
C
C   If poles are used to define boundary velocity
C
      IF(IPOLE.GT.0)CALL VSBSET(XLOAD,QBND,EX,EY,IBC,IBCTYP,
