...
int findfltpts(struct gen_fault **list,struct triangulateio *out,
               int *nfp,int *newvert,int *newseg,int *newibc);
/*void countfltpts(struct gen_fault *flts,int *nfp,
                 int *fltvert,int *newvert,int *newseg); */
             ...
             float pdist,char *infile, char *comments)
{
  ...
  struct gen_fault *flts=0;
  int np,ns,natt,nbm,ndim,nh,nr;
  int err=0;
  int i,j,k;
  int *ncnt=0,*lem,*lemneighbor,*ibc,*ibcngh,*ibctyp,*ifbc,*ifbc2,*polyn,*iseg;
  float *ex,*ey,*vhb;
  float xmin,xmax,ymin,ymax;
  float bbox[4][2];
  extern int pntonsegment_();
  float eps;
  int *numnbs=0,nbpval,a,b,c,d;
  int newvert, newseg, newibc, newbpts, fvert, nbpfinal, mpstart;
  int bvert, bmidpts;
  float *rgnattribs=0,dflt_visc,dflt_sexp,dflt_uvisc,curr;
  float *unodedata=0,*unodeattribs=0;
  ...
  if (*iflt==1 || *iflt==4) triflags.bndpts = 0;
  ...
  *nmp=out.numberofedges;
  newvert=newseg=newibc=newbpts=0;
  bvert=bmidpts=fvert=0;
//if ((*iflt==3 ||*iflt==4) && out.numberofsegments>0) {
  if (*iflt!=0 && out.numberofsegments>0) {
    findfltpts(&flts,&out,nfp,&newvert,&newseg,&newibc); 
    if(!(*nfp)){
        printf("warning:fault type is non-zero but no internal faults found, iflt reset to 0\n");
        *iflt=0;
    }
  }
//  countfltpts no longer used process moved to findfltpts
//  countfltpts(flts,nfp,&fvert,&newvert,&newseg);
    *nfpf3=*nfp;
    newbpts=newseg+newvert;
    fvert=newvert;    // not clear why fvert and newvert
 ...
 // establish the EX and EY arrays
  ...
  ey = *ey_adr;
  ncnt = (int *) malloc(*nup * sizeof(int));
  eps = 1.0e-6;
  for (i = 0; i < *nup; i++) {
    j=i;
    if (j>=*nn) j+=newvert;
  ...
  *nbpext=*nbp;
  ...
  // if periodic boundaries in use, count the nodes prior to allocation of ifbc etc
  if(pbxy>0) countpbpts(nn,&countpb,pbxy,ex,ey,pdist);
  *nfp = *nfp + countpb;    // allow space in fault arrays for pb points
  // printf("after countpbpts: nfpf3 = %i nfp = %i newseg = %i\n",*nfpf3,*nfp,newseg);

  nbpval=0;
  findbbox_(ex,ey,&bbox[0][0],nup);

  /* set XLEN, YLEN from mesh dimensions */
  xmin = bbox[0][0];
  xmax = bbox[1][0];
  *xlen = xmax-xmin;
  ymin = bbox[0][1];
  ...
  *ylen = ymax-ymin;
  for (i = 0; i < *nup; i++) {
    if ((float)(fabs(ey[i]-ymin))<eps || (float)(fabs(ymax-ey[i]))<eps /*||
        (pntonsegment_(&ex[i],&ey[i],&bbox[0][0],&bbox[0][1],
                                   &bbox[3][0],&bbox[3][1])) ||
        (pntonsegment_(&ex[i],&ey[i],&bbox[1][0],&bbox[1][1],
                                   &bbox[2][0],&bbox[2][1]))*/) {
        (nbpval)++;
//printf("%d %f %f\n",i,ex[i],ey[i]);
    }
    /* turn off any errant markers
    else { 
...
//nbpfinal = *nbp+newvert+newseg+newbpts;
  nbpfinal = *nbp+newibc;
//printf("trimesh: ibc array dimensions nbp = %i newibc = %i nbpfinal = %i\n",*nbp,newibc,nbpfinal);
  if ((err=AllocIntArray(nbpfinal,ibc_adr,0))!=0){
  ...
  ibcngh = *ibcnb_adr;
   /* prefer default for zero stress */
  ...
  ibctyp = *ibctyp_adr;
  if (*iflt>0) {
    if ((err=AllocIntArray(*nfp,ifbc_adr,0))!=0){
...
/* this test only valid for rectangular boundaries I think....
    if(nbpval != *nbp && *iflt!=3) {
     printf("boundary points (nbp) %d != points-on-external-boundary (nbpval) %d\n",*nbp,nbpval);
     printf("Tiny elements near the boundary may cause runtime problems\n",*nbp,nbpval);
  } */
  ...
  *ne=out.numberoftriangles;
  ...
  for (i = 0; i < *ne; i++) {
    for (j = 0; j < 3; j++) {
      ...
      if (is_bnd_pt(&out,a)) {
        k=0;
        while (k< *nbp && ibc[k]!=0 && ibc[k]!=a+1) k++;
        if (ibc[k] == 0) ibc[k] = a+1;
        switch (j) {
        /*
...
/*  printf("ibc  array follows, nbp = %i\n",*nbp);
    ...
    /* because basil and triangle order the midpoint nodes differently
                      1                1
                     /|               /|
     ...
     */
  /*
   * this should be simplified - fvert!=newvert if fault goes to bnd
   */
    /*mpstart = bvert+fvert+newvert+(fvert-newvert);*/
    ...
    if (is_bnd_pt(&out,b)) {
        k=mpstart;
        while (k<  nbpfinal && ibc[k]!=0 && ibc[k]!=b+1+newvert) k++;
        if (ibc[k] == 0) ibc[k] = b+1+newvert;
            if (is_bnd_pt(&out,out.trianglelist[i*6+2]))
              ibcngh[k]=out.trianglelist[i*6+2] + 1;
            if (is_bnd_pt(&out,out.trianglelist[i*6]))
              ibcngh[k+nbpfinal]=out.trianglelist[i*6] + 1;
    ...
    if (is_bnd_pt(&out,c)) {
        k=mpstart;
        while (k< nbpfinal && ibc[k]!=0 && ibc[k]!=c+1+newvert) k++;
            if (ibc[k] == 0) ibc[k] = c+1+newvert;
            if (is_bnd_pt(&out,out.trianglelist[i*6]))
              ibcngh[k]=out.trianglelist[i*6] + 1;
            if (is_bnd_pt(&out,out.trianglelist[i*6+1]))
              ibcngh[k+nbpfinal]=out.trianglelist[i*6+1] + 1;
    ...
    if (is_bnd_pt(&out,d)) {
        k=mpstart;
        while (k< nbpfinal && ibc[k]!=0 && ibc[k]!=d+1+newvert) k++;
        if (ibc[k] == 0) ibc[k] = d+1+newvert;
            if (is_bnd_pt(&out,out.trianglelist[i*6+1]))
              ibcngh[k]=out.trianglelist[i*6+1] + 1;
            if (is_bnd_pt(&out,out.trianglelist[i*6+2]))
              ibcngh[k+nbpfinal]=out.trianglelist[i*6+2] + 1;
...
// count node numbers
  ...
  *nmp=0;
  ...
  if(*nup- *nn- *nmp != 0 ) {
    printf(" nup != nn + nmp : nup=%d nn=%d nmp=%d \n",*nup,*nn, *nmp);
    exit(0);
  }
...
//if (*iflt==3  || *iflt==4) {
//  unsigned int closedflt=0;
  if (*iflt){
    int mpstart = out.numberofpoints+newvert-1;
    int vstart = out.numberofpoints-out.numberofedges-1;
    int fstart = -1;
    vxtotal=0;
    while (ibc[vxtotal]!=0) vxtotal++;
    vxtotal--;
    mptotal=nbpfinal-1;
    while (ibc[mptotal]==0) mptotal--;
    vnofault=vxtotal;
    mnofault=mptotal;
    while (flts) {
        if (createafault(&out,&mpstart,&vstart,&fstart,
                lem,ex,ey,ne,ibc,ibcngh,ibctyp,ifbc,ifbc2,iseg,numnbs,
...
/*  printf("ibc  array follows, nbp = %i\n",nbpfinal);
  ...
  *maxnbs=find_maxnbs(ex,ey,lem,*ne,*nn);
  ...
  nbpval=0;
  for (i = 0; i < *nup; i++) {
    if ((float)(fabs(ex[i]-xmin))<eps || (float)(fabs(xmax-ex[i]))<eps /*||
        (pntonsegment_(&ex[i],&ey[i],&bbox[0][0],&bbox[0][1],
                                   &bbox[3][0],&bbox[3][1])) ||
        (pntonsegment_(&ex[i],&ey[i],&bbox[1][0],&bbox[1][1],
                                   &bbox[2][0],&bbox[2][1]))*/) {
  ...
  /*printf("in trimesh, checking for zeros in ibcngh: nbpfinal = %i\n",nbpfinal);
    for (j = 0; j<nbpfinal; j++){
      if ((ibcngh[j]==0)||(ibcngh[j+nbpfinal]==0)){
         printf("j = %i ibc[j] = %i ibcngh[j] = %i ibcngh[j+nbpfinal] = %i\n",
...
int tri_remesh( int *iflt,int *nup,int *nn,int *nmp,int *nbp,int *nnnof,
             int *nmpnof,int *ne,int *nfp,int *nseg,int *ivv,int *maxnbs,
             int *nellep,int **ielle_adr,int **ipolyn_adr,
             int **lem_adr,int **lemneighbor_adr,
             int **ibc_adr,int **ibcnb_adr,int **ibctyp_adr,int **ifbc_adr,int **ifbc2_adr,
             ...
             )
{
  ...
  int np,ns,natt,nbm,ndim,nh,nr;
  int i,j,k;
  int *ncnt=0,*lem,*lemneighbor,*ibc,*ibcngh,*ibctyp,*ifbc,*ifbc2,*ibpole,*iseg,*imat;
  float *ex,*ey,*qbnd,*taperf;
  float xmin,xmax,ymin,ymax;
  float bbox[4][2];
  extern int pntonsegment_();
  float eps;
...
/*int maxrgns=0,maxunodes, dist_set=0; */
  int clrptr=0,icount=0,vxtotal=0,mptotal=0;
  int numrgns = 0;
  extern float RealVars[64];
  int *nbpext;
...
if  (mreg>0) {
  ...
  if (tripolydata(&out,&inp,&triflags,newnbp,newnseg,num_pt_attr,
                  isegnew,exbnd,eybnd,pt_attributes,rgnattribs,numrgns)!=0)
      return(1);
  /* WritePoly(&inp, "regrid.poly"); */

    if (numrgns>0) {free(rgnattribs); rgnattribs=0;}

    if (*lem_adr!=0) free(*lem_adr); *lem_adr=0;
    if (*lemneighbor_adr!=0) free(*lemneighbor_adr); *lemneighbor_adr=0;
    if (*ibc_adr!=0) free(*ibc_adr);*ibc_adr=0;
    if (*ibcnb_adr!=0) free(*ibcnb_adr); *ibcnb_adr=0;
    if (*ibctyp_adr!=0) free(*ibctyp_adr); *ibctyp_adr=0;
    if (*ibpole_adr!=0) free(*ibpole_adr); *ibpole_adr=0;
    if (*iseg_adr!=0) free(*iseg_adr); *iseg_adr=0;
    if (*imat_adr!=0) free(*imat_adr); *imat_adr=0;
    if (*qbnd_adr!=0) free(*qbnd_adr); *qbnd_adr=0;
    if (*taperf_adr!=0) free(*taperf_adr); *taperf_adr=0;
  /**********************************/
  /******* Setup Basil Arrays *******/
  /**********************************/
  ...
  *nup=out.numberofpoints;
  if ((out.numberofcorners=6)){
    ...
    *nmp=0;
    /*but Basil expects midpts so flag error */
    printf("no midpoint nodes found\n");
    return(1);
  }
  newvert=newseg=newbpts=0;
  bvert=bmidpts=fvert=0;
  if ((*iflt==3 || *iflt==4) && out.numberofsegments>0) {
    if (findfltpts(&flts,&out,nfp,&newvert,&newseg,&newibc)) {
        printf("Warning: fault type is 3 but no internal points found\n");
    }
    /*
     *  fvert is current flt vertices
     *  which are not external bnd pts
     *  current flt midpts == newseg
     */
 // countfltpts(flts,nfp,&fvert,&newvert,&newseg);
    newbpts=newseg+newvert;
    fvert=newvert;    // not clear why fvert and newvert
  }
  else if (*iflt==3 || *iflt==4) {
      printf("Warning: fault type is 3 or 4 but no fault segments found\n");
  }
...
/* printf("\n"); fflush(stdout); */
  nbpfinal = *nbp+fvert+newseg+newbpts;/* *nbp only includes ext bnd pts */
  ...
  *ifbc_adr = ifbc = (int *) malloc(*nfp * sizeof(int));
  *ifbc2_adr = ifbc2 = (int *) malloc(*nfp * sizeof(int));
  *ibpole_adr = ibpole = (int *) malloc(nbpfinal * 2 * sizeof(int));
  *qbnd_adr = qbnd = (float *) malloc(nbpfinal * 2 * sizeof(float));
  *taperf_adr = taperf = (float *) malloc(nbpfinal * sizeof(float));
  for (i=0; i < nbpfinal; i++) ibc[i]=0;
  for (i=0; i < nbpfinal * 2; i++) ibcngh[i]=0;
  for (i=0; i < nbpfinal * 2; i++) ibctyp[i]=-1;
  for (i=0; i < (*nfp); i++) ifbc[i]=0;
  for (i=0; i < (*nfp); i++) ifbc2[i]=0;
  for (i=0; i < nbpfinal * 2; i++) ibpole[i]=0;
  for (i=0; i < nbpfinal * 2; i++) qbnd[i]=0;
  for (i=0; i < nbpfinal; i++) taperf[i]=0;

/*
  if(nbpval != *nbp && *iflt!=3) {
     printf("The number of boundary points differs from the number of points that pass a point-on-external-boundary test: %d != %d\n",*nbp,nbpval);
     printf("Tiny elements near the boundary may cause runtime problems\n",*nbp,nbpval);
     exit();
  } */
/*
  ...
  *lemneighbor_adr = lemneighbor= (int *) malloc(*ne * 3 * sizeof(int));
  ...
  if (imat!=NULL && out.triangleattributelist!=NULL) {
      for (i = 0; i < *ne; i++) {
          ...
          if((imat[i]<=0)||(imat[i]>=5))icount=icount+1;
      }
      printf("tri_remesh: IMAT has %i values ouside range 1-4\n",icount);
  }
#if XY
  /*
   * ivv    visc (vhb[i,1-7])    se (vhb[i,8])
  ...
  /***** count the max number of nodes connected to a vertex ******/
  *maxnbs=0;
  for (i = 0; i < *nn+newvert; i++) if (numnbs[i] > *maxnbs)
      *maxnbs=numnbs[i];
    /*  printf("maxnbs=%d\n",*maxnbs); */

  /***** count vertex and midpoint nodes ******/
  /***** midpoint nodes= number of edges, vertex=pts-midpts ******/
  *nn=0;
  *nmp=0;
  for (i = 0; i < *nup; i++) {
    if(ncnt[i]==0) (*nn)++;
  ...
  if (out.numberofsegments>0 && out.segmentlist) {
    *nseg = out.numberofsegments;
    if ((*iseg_adr = iseg =
                  (int *) malloc((*nseg+newseg) * 3 * sizeof(int)))==NULL)
          return(1);
    for (i = 0; i < *nseg; i++) {
        iseg[i*3]=out.segmentlist[i*2]+1;
        iseg[i*3+1]=out.segmentlist[i*2+1]+1;
  ...
  /***** store bnd data - boundaries defined in .poly file ******/
  /***** qbnd and ibctyp are passed via pt attribute data (defined real)
         Note probable inconsistency at boundary condition discontinuities
         where ibctyp changes from 0 to 1 and QBND values are linearly
         interpolated for points that are added by triangle, in
         particular a problem may arise at a corner 
         This issue not cleanly resolved   *******/

  if (out.numberofpointattributes==5) {
    for (i=0;i<nbpfinal;i++) {
       j=ibc[i]-1-newvert;
       ibctyp[i]=(int)ceilf((float)(out.pointattributelist[j*out.numberofpointattributes+2]));
       ibctyp[i+nbpfinal]=(int)ceilf((float)(out.pointattributelist[j*out.numberofpointattributes+3]));
/*     ibctyp[i]=(int)(out.pointattributelist[j*out.numberofpointattributes+2]+1.e-5);
       ibctyp[i+nbpfinal]=(int)(out.pointattributelist[j*out.numberofpointattributes+3]+1.e-5); */
       qbnd[i]=out.pointattributelist[j*out.numberofpointattributes];
       qbnd[i+nbpfinal]=out.pointattributelist[j*out.numberofpointattributes+1];
       if (ibctyp[i]==1 && fabs(qbnd[i])>1e-6)
          qbnd[i]=0.0;
       if (ibctyp[i+nbpfinal]==1 && fabs(qbnd[i+nbpfinal])>1e-6)
          qbnd[i+nbpfinal]=0.0;
    }
  }
  else if (out.numberofpointattributes==8) { //ipole data added
    for (i=0;i<nbpfinal;i++) {
       j=ibc[i]-1-newvert;
       qbnd[i]=out.pointattributelist[j*out.numberofpointattributes];
       qbnd[i+nbpfinal]=out.pointattributelist[j*out.numberofpointattributes+1];
...
/*     ibctyp[i]=(int)(out.pointattributelist[j*out.numberofpointattributes+2]+1.e-5);
       ibctyp[i+nbpfinal]=(int)(out.pointattributelist[j*out.numberofpointattributes+3]+1.e-5); */
       ibpole[i]=(int)(out.pointattributelist[j*out.numberofpointattributes+4]+1.e-5);
       ibpole[i+nbpfinal]=(int)(out.pointattributelist[j*out.numberofpointattributes+5]+1.e-5);
       taperf[i]=out.pointattributelist[j*out.numberofpointattributes+6];
       if (ibctyp[i]==1 && fabs(qbnd[i])>1e-6)
          qbnd[i]=0.0;
       if (ibctyp[i+nbpfinal]==1 && fabs(qbnd[i+nbpfinal])>1e-6)
          qbnd[i+nbpfinal]=0.0;
  ...
  if (*iflt==3 || *iflt==4) {
    ...
    int fstart = -1;
    while (flts) {
        closedflt= (flts->id > 2500) ? 1 : 0;
/*      if (closedflt==0) {
            if (createfault(&out,&mpstart,&vstart,
                    lem,ex,ey,ne,ibc,ibcngh,ibctyp,iseg,numnbs,
                    nbpfinal,nseg,*maxnbs,flts)) {
                printf("error creating fault\n");
                return(1);
            }
        }
            ...
            if (createafault(&out,&mpstart,&vstart,&fstart,
                    lem,ex,ey,ne,ibc,ibcngh,ibctyp,ifbc,ifbc2,
                    iseg,numnbs,nbpfinal,nbpext,nseg,&vxtotal,&mptotal,flts)) {
                printf("error creating fault\n");
                return(1);
            }
//      }
...
/* #if XY
    if (createfaults(&out,lem,ex,ey,ne,ibc,ibcngh,ibctyp,iseg,numnbs,
                    nbpfinal,nseg,*maxnbs,newvert,flts)) {
        printf("error creating fault\n");
        return(1);
    }
#endif  */
    ...
    *nmp += newseg;
    *nup += newbpts;
    for (i = 0; i < *nn; i++) if (numnbs[i] > *maxnbs)
      *maxnbs=numnbs[i];
                        ...
                        int *maxunodes,float **unodedata)
{
    ...
    int found = 0, finished=0, i;
    int num=0;
    float *ptr_x=0, *ptr_y=0, *ptr_rgn=0;
...
#endif
{
  ...
  float val;
  float eps;
  float *rgnattribs=0,dflt,curr;
  double ppts[6], pt[2];
  int maxrgns,ne=0,found,num=3;
  int dum=0,*dum_adr;
/******************
...
{
    unsigned char found;
    int fltid, start, curr, prev, next, nb, midpt;
    int end1, end2;
    int *nbs=0, *tri_nbs, cnt, tricnt, err=0;
    int i,j,k,m;
    struct node *fltnode;
    int vindex, mpindex, segstart, lemindx;

    segstart = *nseg;

    if ((nbs = (int *) malloc(maxnbs * sizeof(int)))==NULL)
          return(1);
    if ((tri_nbs = (int *) malloc(maxnbs * sizeof(int)))==NULL)
          return(1);
    vindex=0;
    while (ibc[vindex]!=0) vindex++;
    vindex--;
    mpindex=nbpfinal-1;
    while (ibc[mpindex]==0) mpindex--;
    fltid=flt->id;
    end1=flt->end1;
    end2=flt->end2;
    fltnode=flt->ordered_nodes;
    start=curr=prev=fltnode->id;
    while (fltnode->next) {
        next=fltnode->next->id;
        iseg[*nseg*3] = curr+1;
        // nbs does not include prev or next, only nbs between
        findNeighbourPts(curr, prev, next, out, &cnt, nbs, maxnbs);
        if (cnt<1) { nbs[0]=next; cnt++; }
        //if (cnt<1)
        //  printf("error finding flt point neighbours for %d\n",curr);
            // find triangles which will get new segments
        tricnt=0;
        findTriangles(curr,nbs,cnt,lem,*ne,tri_nbs,&tricnt);
        if (tricnt<=0 || (curr==flt->end1&&cnt!=tricnt) || 
                    (fltnode->next!=0&&curr!=flt->end1&&cnt!=tricnt-1)) {
            printf("createfault: Problem finding triangles from nbs\n");
            //return(1);
        }
                // 
                 * if flt end on extrnl bnd or not at an end, duplicate vertex

        printf("createfault: curr = %i end1 = %i end2 = %i\n",curr,end1,end2);
        if (curr==end1  ) {
                //if (curr!=start)
            vindex++;
            (*vstart)++;
            ...
            ey[*vstart]=ey[curr];
            ibc[vindex]=*vstart+1;
            iseg[(*nseg)*3]=*vstart+1;
            iseg[(*nseg)*3+1]=next+1;
            nb = nbs[cnt-1];
            for(k=0;k<*nseg;k++) {
                if (iseg[k*3]==curr+1 && iseg[k*3+1]==nb+1)
                    iseg[k*3]=*vstart+1;
                else if (iseg[k*3+1]==curr+1 &&
                                         iseg[k*3]==nb+1)
                    iseg[k*3+1]=*vstart+1;
            }
            ibcngh[vindex]=nb+1;
            ibcngh[vindex+nbpfinal]=next+1;
            for(k=0;k<=nbpfinal;k++) {
                if (ibcngh[k]==curr+1 &&
                                ibcngh[k+nbpfinal]==nb+1)
                     ibcngh[k]=*vstart+1;
                else if (ibcngh[k]==nb+1 &&
                                ibcngh[k+nbpfinal]==curr+1)
                     ibcngh[k+nbpfinal]=*vstart+1;
            }  
            for(k=0;k<=vindex;k++) {
                if (ibc[k]==curr+1 && ibcngh[k]==nb+1)
                     ibcngh[k]=next+1;
                if (ibc[k]==nb+1 && ibcngh[k+nbpfinal]==curr+1)
                     ibcngh[k+nbpfinal]=*vstart+1;
            }  
            for (i=0;i<tricnt;i++) {
                for (j=0,found=0;j<3 && !found;j++) {
                    if (lem[tri_nbs[i]*6+j]==curr+1) {
                        found=1;
                        lemindx=j;
                        lem[tri_nbs[i]*6+j]=*vstart+1;
                    }
                 }
            }
        }
            ...
            vindex++;
            ibc[vindex]=curr+1;
            ibcngh[vindex]=next+1;
            ibcngh[vindex+nbpfinal]=prev+1;
            (*vstart)++;
            vindex++;
            ex[*vstart]=ex[curr];
            ey[*vstart]=ey[curr];
            ibc[vindex]=*vstart+1;
            ibcngh[vindex]=*vstart;
            ibcngh[vindex+nbpfinal]=next+1;
            iseg[(*nseg)*3]=*vstart+1;
            iseg[(*nseg)*3+1]=next+1;
            for(k=0;k<*nseg;k++) {
                if (iseg[k*3]==curr+1 && iseg[k*3+1]==*vstart)
                    iseg[k*3]=*vstart+1;
                else if (iseg[k*3+1]==curr+1 && iseg[k*3]==*vstart)
                    iseg[k*3+1]=*vstart+1;
            }
            if (prev==start && prev!=end1)
                iseg[(*nseg-1)*3+1]=*vstart+1;
                //ibcngh[mpindex-1+nbpfinal]=*vstart+1;
            for (i=0;i<tricnt;i++) {
                for (j=0,found=0;j<3 && !found;j++) {
                    if (lem[tri_nbs[i]*6+j]==curr+1) {
                        found=1;
            ...
            for(k=0;k<=mpindex;k++)
                    if (ibc[k]==*vstart||ibc[k]==*mpstart+1)
                        ibcngh[k+nbpfinal]=*vstart+1;
        }
        else { // curr==start but not an ext bnd pt

           for (i=0,found=0;i<=vindex && !found;i++) 
                if (ibc[i]==curr+1) {found=1; j=i; }
            if (!found) {
                vindex++;
                j=vindex;
                ibc[j]=curr+1;
            ...
            ibcngh[j]=next+1;
            ibcngh[j+nbpfinal]=next+1;
            if (fltid<=2500) ibctyp[j]=15;
            if (fltid<=2500) ibctyp[j+nbpfinal]=15;
            iseg[(*nseg)*3]=curr+1;
            iseg[(*nseg)*3+1]=next+1;
            numnbs[curr]++; // maxnbs may increase
        }
            // duplicate midpt
        for (k=0,lemindx=-1,found=0;k<tricnt && !found;k++) {
            m=tri_nbs[k];
            if (lem[m*6]==next+1) {
                j=0;
                if (lem[m*6+1]==curr+1||lem[m*6+1]==*vstart+1) lemindx=1;
                else if (lem[m*6+2]==curr+1||lem[m*6+2]==*vstart+1) lemindx=2;
            }
            else if (lem[m*6+1]==next+1) {
                j=1;
                if (lem[m*6]==curr+1||lem[m*6]==*vstart+1) lemindx=0;
                else if (lem[m*6+2]==curr+1||lem[m*6+2]==*vstart+1) lemindx=2;
            }
            else if (lem[m*6+2]==next+1) {
                j=2;
                if (lem[m*6]==curr+1||lem[m*6]==*vstart+1) lemindx=0;
                else if (lem[m*6+1]==curr+1||lem[m*6+1]==*vstart+1) lemindx=1;
            }
            if (lemindx!=-1) found=1;
        }
        if (found==1) {
            switch(lemindx) {
            case 0: if (j==2) k=3;
            ...
            case 2: if (j==1) k=5;
                    ...
                    midpt=lem[m*6+k]-1;
                     break;
            default: printf("midpt not found\n");
                     return(1);
                     break;
            }
            mpindex++;
            ibc[mpindex]=midpt+1;
            ibcngh[mpindex]=next+1;
            ibcngh[mpindex+nbpfinal]=curr+1;
   printf("Adding midpt %d index %d\n",midpt+1,mpindex);
            mpindex++;
            (*mpstart)++;
            lem[m*6+k]=*mpstart+1;
            ex[*mpstart]=ex[midpt];
   ...
   printf("Setting ibc[%d] to %d\n",mpindex,*mpstart+1);
            //
             * internal starting point - nb is not new vertex

            if (curr==start && curr!=end1)
                ibcngh[mpindex]=curr+1;
            else
                ibcngh[mpindex]=*vstart+1;
            ibcngh[mpindex+nbpfinal]=next+1;
        }
        iseg[(*nseg)*3+2]=fltid;
       (*nseg)++;
        prev = curr;
    ...
    tricnt=0;
    findTriangles(curr,nbs,cnt,lem,*ne,tri_nbs,&tricnt);
    if (tricnt<=0 || (curr==flt->end2&&cnt!=tricnt) || 
                                 (curr!=flt->end2&&cnt!=tricnt-1)) {
            printf("Problem finding triangles from nbs\n");
            //return(1);
    }
        // if flt end on extrnl bnd, duplicate vertex
    if (curr==flt->end2) {
        (*vstart)++;
        vindex++;
        ex[*vstart]=ex[curr];
        ...
        iseg[(*nseg-1)*3+1]=*vstart+1;
        nb=nbs[cnt-1];
        for(k=0;k<segstart;k++) {
            if (iseg[k*3]==curr+1 && iseg[k*3+1]==nb+1)
                iseg[k*3]=*vstart+1;
            else if (iseg[k*3+1]==curr+1 && iseg[k*3]==nb+1)
                iseg[k*3+1]=*vstart+1;
        }
        ibc[vindex]=*vstart+1;
        ibcngh[vindex]=*vstart;
        ibcngh[vindex+nbpfinal]=nb+1;
        for(k=0;k<=mpindex;k++)
            if (ibc[k]==*vstart||ibc[k]==*mpstart+1)
                ibcngh[k+nbpfinal]=*vstart+1;
        for(k=0;k<=nbpfinal;k++) {
            if (ibcngh[k]==curr+1 && ibcngh[k+nbpfinal]==nb+1)
                 ibcngh[k]=*vstart+1;
            if (ibcngh[k]==nb+1 && ibcngh[k+nbpfinal]==curr+1)
                 ibcngh[k+nbpfinal]=*vstart+1;
        }  
        for(k=0;k<=vindex;k++) {
            if (ibc[k]==curr+1 && ibcngh[k]==nb+1)
                ibcngh[k+nbpfinal]=prev+1;
            if (ibc[k]==nb+1 && ibcngh[k]==curr+1)
                 ibcngh[k]=*vstart+1;
        } 
        for (i=0;i<tricnt;i++) {
            for (j=0,found=0;j<3 && !found;j++) {
    ...
    else { // curr==end but not an ext bnd pt
        for (i=0,found=0;i<=vindex && !found;i++) 
            if (ibc[i]==curr+1) {found=1; j=i; }
        if (!found) {
            if (curr!=start) vindex++;
            j=vindex;
            ibc[j]=curr+1;
        }
        ibcngh[j]=*vstart+1;
        ibcngh[j+nbpfinal]=prev+1;
        ibctyp[j]=15;
        ibctyp[j+nbpfinal]=15;
        numnbs[curr]++; // maxnbs may increase
    }
//  for(k=segstart;k<*nseg;k++) 
//          printf("new seg %d %d %d\n",
//                      iseg[k*3],iseg[k*3+1],iseg[k*3+2]);
    if (nbs) free(nbs);
    if (tri_nbs) free(tri_nbs);
    return(err);
} */
...
int createafault(struct triangulateio *out,
                 ...
                 int *ibc,int *ibcngh,int *ibctyp,int *ifbc,int *ifbc2,
                 int *iseg,int *numnbs,
                 int nbpfinal,int *nbpext,int *nseg,int *vxtotal,int *mptotal,
                 struct gen_fault *flt)
...
// ifbc and ifbc2 are now established in this routine.
// ibctyp for all fault nodes is initialised to 11
{
    int fltid, start, curr, next, away, first, last, nb, midpt, curr1, curr2, next1, next2;
    int *nbs=0, *tri_nbs, *tri_1=0, *tri_2=0, *tri_3=0, *midpts=0, *newnext=0;
    int nums, count, i, j, k, nelf, inxt, nnxt, itmp, nsg1, err=0;
    int nel1, nel2, j1curr, j1next, j1midp, j2curr, j2next, j2midp, midpt1;
    int tmplem, tmpnxt, tmpmid, faultclosed=0;
    int vindex, mpindex, vfirst, mfirst, segstart;
    int ift1,ift2,kn,ierr=0;
    struct node *fltnode;
...
//  followingg operation done once before sequence of calls to createafault
    ...
    mpindex=nbpfinal-1;
    while (ibc[mpindex]==0) mpindex--;
//  printf("entering createafault: vindex = %i mpindex = %i\n",vindex,mpindex);
    fltid=flt->id;
    fltnode=flt->ordered_nodes;
    nums=0; 
...
// if sequence of nodes is in anti-clockwise order.
//
    curr=fltnode->id+1;
    for(k=0;k<nums;k++){
  ...
  //    if current fault is being joined to an earlier fault on the wrong side.
              for(kn = 0; kn < *fstart; kn++){
                 if(ifbc[kn]==curr){
                    ...
                    curr=-ifbc[kn+1];
  //                printf("kn = %i curr = %i ifbc[kn+1] = %i\n",kn,curr,ifbc[kn+1]);
                 }
                 else if(ifbc[kn]==next){
                    printf("Substituting fault pair node %i for node next = %i\n",-ifbc[kn+1],next);
                    next=-ifbc[kn+1];
  //                printf("kn = %i next = %i ifbc[kn+1] = %i\n",kn,next,ifbc[kn+1]);
                 }
              }
              nelf=findTriangV(k,curr,next,lem,*ne,tri_1,tri_2);
              if(nelf==2)ierr=0;
 ...
 // printf("before new vertices, *mpstart = %i *vstart = %i, vindex = %i\n",*mpstart,*vstart,vindex);
    //  work through the list of segments identified in tri_1 and tri_2
	   //   in each pass of this loop we focus on changing things related to the 
	   //   'next' vertex fault node (== 'curr' fault node of the next pass)
	   //   for an open fault, we don't add the last 'next' vertex
	   //   -----curr-----next----    ----  at step i
	   //   -----    -----curr----next----  at step i+1
	   //   for open fault, end points are not duplicated unless on external boundary
 ...
 //     printf("createafault fltid = %i curr = %i next = %i\n",fltid,curr,next); 
        if((i==0) && ift1){     // duplicate first point if node ift1 already in ibc
	     ...
	     ex[*vstart]=ex[curr-1];
    	     ey[*vstart]=ey[curr-1];
	     ibc[vindex]=*vstart+1;       // duplicate node added
             ibctyp[vindex]=11;
             ibctyp[vindex+nbpfinal]=11;
             (*fstart)++;
             ifbc[*fstart]=-(*vstart+1);
   ...
   //   for the existing vertex node on the + side of the fault
	...
	if((next != last) || faultclosed || ift2){
             (*fstart)++;                 // add ifbc entries, also if boundary reached
             ifbc[*fstart]=next;
             ifbc2[*fstart]=vindex+1;
   ...
   //   note that special treatment for ends of faults depends if they reach
   //   the external boundary or not
    for(i=0; i<nums; i++){
	nel1=tri_1[2*i];
	j1curr=tri_1[2*i+1];
	j1next=(j1curr+1) % 3;
	...
	next2=lem[6*nel2+j2next];
        for(j=0; j<vindex; j++){                  // all of the V nodes in ibc
           if (ibc[j]==curr1){
              ibcngh[j+nbpfinal]=next1;           // anti-clockwise connection
              if(i==0){
                 if(ift1){
                    ibcngh[ift1]=next1;           // connect ift1
                 }
                 else{
                    ibcngh[j]=next2;              // connect to unattached fault tip
                 }
              }
           }
           if (ibc[j]==next1){                    // clockwise connection
              ibcngh[j]=curr1;
              if(i==(nums-1)){
                 if(ift2){
                    ibcngh[ift2+nbpfinal]=curr1;  // connect ift2
                 }
                 else{
                    ibcngh[j+nbpfinal]=curr2;     // connect to unattached fault tip
                 }
              }
           }
    // if i = 0 curr1 and curr2 should be the same node for an unattached fault
    // if i = nums-1 next1 and next2 should be the same node for an unattached fault
           if (ibc[j]==curr2){ 
              ibcngh[j]=next2;                    // clockwise connection
              if((i==0)&&(ift1)){                 // connection to ift1 neighbour
                 ibcngh[j+nbpfinal]=ibcngh[ift1+nbpfinal];
              }
           }
           if (ibc[j]==next2){
              ibcngh[j+nbpfinal]=curr2;           // anti-clockwise connection
              if((i==(nums-1))&&(ift2)){          // connection to ift2 neighbour
                 ibcngh[j]=ibcngh[ift2];
              }
           }
        }

   // final task, locate and replace entries in ibcngh for external boundary nodes replaced
   // search all entries in ibcngh; only the ones that need to be changed will be found
        ...
        if((i==0)&&(ift1)){
           for (j=0; j<*nbpext; j++){
              if(ibcngh[j]==curr1){ 
                 ...
                 for (k=0; k<*nseg; k++){
                    if(iseg[k*3+2]!=fltid){
                       if((iseg[k*3]==curr1)&&(iseg[k*3+1]==ibc[j])){
                          iseg[k*3]=curr2;}
                       if((iseg[k*3+1]==curr1)&&(iseg[k*3]==ibc[j])){
                          iseg[k*3+1]=curr2;}
                    }
                 }    // for k
              }
           }          // for j
        }             // if beginning of fault on pre-defined structure
        if((i==(nums-1))&&(ift2)){
           for (j=0; j<*nbpext; j++){
              if(ibcngh[j+nbpfinal]==next1){ 
                 ibcngh[j+nbpfinal]=next2;
                 for (k=0; k<*nseg; k++){
                    if(iseg[k*3+2]!=fltid){
                       if((iseg[k*3]==next1)&&(iseg[k*3+1]==ibc[j])){iseg[k*3]=next2;}
                       if((iseg[k*3+1]==next1)&&(iseg[k*3]==ibc[j])){iseg[k*3+1]=next2;}
                    }
                 }     // for k
              }
           }           // for j
        }              // if end of fault on pre-defined structure
    }
 ...
 /* int newvnodes=2*(nums-1);
    if(ift1){newvnodes++;}
    if(ift2){newvnodes++;}
    printf("new vertexnodes : %i\n",newvnodes);
    for(i=0; i<newvnodes; i++){
       itmp=vfirst+1+i;
...
//  in fortran entities
{
    int i,j,m,next,id,fltid,cnt,cntv,err=0;
    int finished, start, faultclosed, segno;
    int fltMIN, fltMAX, extMIN, extMAX, oldmarker;
    int *fltsegs=0;
    int ift1,ift2,met1,met2,ifx1,ify1,ifx2,ify2,setn;
    struct gen_fault *startlist, *list;
    struct gen_fault *nfnode=0;
    struct node *nnode=0, *pos, *startpos;
    ...
    *nfp = *newvert = *newseg = *newibc = 0;
    ...
    bndextlimits_(&extMIN,&extMAX);
    for (i = 0; i < out->numberofsegments; i++) {
        /* initialise a list for each fault */
        if (out->segmentmarkerlist[i] >= fltMIN    &&
                out->segmentmarkerlist[i] <= fltMAX &&
    ...
    startlist=list=*fltlist;
    while (list && !err) {
      
  //    for each fault, count the number of segments
        fltid=list->id;
        fltcnt++;
        cnt=0;
        for (i=0;i<out->numberofsegments && !err;i++)
            if (out->segmentmarkerlist[i]==fltid) cnt++;
        if (cnt<3) {
            printf("Need a minimum of 3 vertex points to create fault %d\n",
                            fltid);
            err=1;
        }
  //    printf("findfltpts: for fltid = %i, cnt = %i\n",fltid,cnt);
        *newseg=*newseg+cnt;
        *newvert=*newvert+cnt-1;
        *newibc=*newibc+cnt*4;  // 2 sets of vertices + midpoints, + 2 end vertices
  //    newvert and newibc here correct for open fault unconnected to prior structures
  //    newibc also correct for closed fault;    both may be amended below
  
  //    store the segment indices to help find the nodes
  //    printf("findfltpts: fltid = %i no. of segs = %i\n",fltid,cnt);
        if ((fltsegs = (int *) malloc(cnt*2 * sizeof(int)))==NULL)
          return(1);
        for (j=0;j<cnt*2;j++) fltsegs[j] = 0;
  ...
  //    unless closed, in which case start point is arbitrary
        ift1=0;
        ift2=0;
        faultclosed=0;
        for (j=0; j<cnt; j++) {
            ifx1=out->segmentlist[fltsegs[j]*2];
            ify1=out->segmentlist[fltsegs[j]*2+1];
            met1=ifx1;
            met2=ify1;
            for (m=0; m<cnt; m++) {
               if(m!=j){
                  ifx2=out->segmentlist[fltsegs[m]*2];
                  ify2=out->segmentlist[fltsegs[m]*2+1];
                  if (met1 && ((ifx1==ifx2)||(ifx1==ify2)))met1=0;
                  if (met2 && ((ify1==ify2)||(ify1==ifx2)))met2=0;
               }
            }
  //  ift1 and ift2 are the start and end nodes for the fault
            if(ift1 && !ift2){
                if(met1 && (ift1 != met1)){ift2=met1;}
                if(met2 && (ift2 != met2)){ift2=met2;}
            }
            if(!ift1){
               if(met1){ift1=met1;}
               if(met2){ift1=met2;}
  ...
  //   if the fault is closed, first point is arbitrary, extra vertex point
        if((ift1==0) && (ift2==0)){
          faultclosed=1;
          (*newvert)++;
  //      *newibc = *newibc + 2;
  ...
  //    printf("findfltpts: fltid = %i ift1 = %i ift2 = %i id = %i\n",
  //                       fltid,ift1,ift2,id);
   ...
   //  and add points according to connections
    ...
    //           printf("j = %i setn = %i id = %i next = %i\n",j,setn,id,next);
                 if ((nnode=newnode(next))==0) return(1);
                 pos->next = nnode;
                 pos = nnode;
                 id = next;
                 finished++;
   ...
   //   add final point and close list
        ...
        cntv=cnt;
        if(!faultclosed)cntv--;
   //   printf("findfltpts: fltid = %i new segs = %i new verts = %i\n",fltid,cnt,cntv);
   ...
   // (i.e. lesser fltid), in which case additional node is added there
   // newibc does not need amendment because existing node already counted
   // but extra node is added
        if(ift1){
           found=0;
           ...
           if(found){
              (*newvert)++;    // extra node added to open fault
           // printf("node %i in fault %i also in boundary %i vert++\n",
           //         ift1+1,fltid,found);
           }
        }
        ...
        if(ift2){
           ...
           if(found){
              (*newvert)++;    // extra node added on open fault
           // printf("node %i in fault %i also in boundary %i vert++\n",
           //         ift2+1,fltid,found);
           }
        }
 ...
 //     printf("findfltpts: fltid = %i newvert = %i newseg = %i newibc = %i\n",fltid,*newvert,*newseg,*newibc);
        list=list->next;
    }  // end of list of faults
    printf("Faults identified: %i with newvert = %i newseg = %i newibc = %i\n",fltcnt,*newvert,*newseg,*newibc);

 // space to be allocated for fault nodes 
    (*nfp) = (*newseg + *newvert)*2;
    return(0);
}   // end of findfltpts

/*  countfltpoints replaced by same functionality in findfltpts
 * count the number of fault pts to be duplicated (newvert+newseg)
 * checks for faults which end on an external boundary but
 * not yet accounting for fault intersection points
 * endpts which are internal are not duplicated but are entered in ibc
 * so they are existing and incl in fltvert but not in newvert or nfp
 */
/* void countfltpts(struct gen_fault *list,int *nfp,
         int *fltvert,int *newvert,int *newseg)
{
    struct node *nnode;
    int node_cnt;
// GH changes 30/12/21: net effect to reduce fltvert by 2 for open fault
    *nfp = *fltvert = *newvert = *newseg = 0;
    while (list) {
      if (list->id < 2500) {                        // open fault
        nnode=list->ordered_nodes;
        if (list->end1==nnode->id) (*newvert)++;    // external end
        (*newseg)++;
        nnode=nnode->next;
        while (nnode->next) {
            (*newvert)++;
        ...
        if (list->end2==nnode->id) (*newvert)++;     // external end
      }
      else{                                      // closed fault
        node_cnt = 0;
        nnode=list->ordered_nodes;
        while (nnode->next!=0 && nnode->next!=list->ordered_nodes)
            { nnode=nnode->next; node_cnt++; }
        *fltvert += node_cnt;
        *newvert += node_cnt;
        *newseg += node_cnt;
      }
...
unsigned char is_ext_bnd_pt(int id, int *iseg, int nseg)
{
    ...
    /* limits for boundary id numbers for external bnds */
    bndextlimits_(&extMIN,&extMAX);
    for (i=0;i<nseg && !found;i++) {
      /*
       * iseg set up for fortran (1,nseg) but lists use
       * C indexing (0,nseg-1) so id=index-1
       */
        if (iseg[i*3]-1 == id || iseg[i*3+1]-1 == id)
            if (iseg[i*3+2]>=extMIN && iseg[i*3+2]<=extMAX) found=1;
    }
    return(found);
}
