...
REAL area;                      /* max allowed area of a triangle. */
...
dxao = tapex[0] - torg[0];
dyao = tapex[1] - torg[1];
/*  area = -0.5*(dxod*dyao - dxao*dyod)  as used below */
dxod2 = dxod * dxod;
dyod2 = dyod * dyod;
dxda2 = dxda * dxda;
dyda2 = dyda * dyda;
...
/*  area formula changed by GH - Mar 31, 2016 */
area = -0.5*(dxod*dyao - dxao*dyod);
if (b->fixedarea && (area > b->maxarea)) {
/* fprintf(stdout," in testtriangle, area = %f b.maxarea = %f\n",area,b->maxarea); */
...
triangleinit(&m);
#ifdef TRILIBRARY
/* fprintf(stdout,"in triangle.c, calling parsecommand, triswitches = %s \n",
           triswitches);  */
