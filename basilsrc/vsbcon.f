...
C
     ...
     2       (IFLT.EQ.2.AND.ABS(XT2-XLEN).LT.EPS))THEN
C
...
C
     ...
     :                  POLEP,IPOLE,IBPOLE,TAPERF,ARGAN,HLENSC,
...
C
      XMID=CENTLNG
C     GO TO 50       ! annuls setting of BC's - only for debugging
      LENGTH=LINELEN*2
...
C
     ...
     :              ARGAN,HLENSC,CENTLNG,IDEFTYP,NOR,LEM,IBC,
...
C
     ...
     :              CENTLNG,ROTIX,DPHI,NOR,LEM,IBC,IBNGH,IBCTYP,NCOMP,
     :              NUP,NE,NN,NBP,XPT0,YPT0,POLEP,IBPOLE,TAPERF,
...
10101   FORMAT(' ',/,'Array data from VSBCON:',/,
     ...
     :  '    TYPY   QBND_Y')
        DO 600 J=1,NBP
          ...
          WRITE(LUW,10102)J,QEX,QEY,IBC(J),
     1               IBNGH(J),IBNGH(JP),IBCTYP(J),UU,IBCTYP(JP),VV
10102     FORMAT(I5,2F9.3,3I6,I3,G14.5,I3,G14.5)
  600   CONTINUE
C
      RETURN
      END
...
C
      ...
      PMID=2.0/3.0
      PVERT=1.0/6.0
C   earlier versions adjusted the boundary tractions relative to Ar to 
C   simplify running systematic experiments in a specific class of 
C   problems.  In general adjustment is not appropriate and is removed.
C     CT0=ARGAN/(HLENSC*HLENSC)
      CT0=1.0
      IYX=' '
      IXY='   '
      IUT=IXY
...
C
     ...
     :                  CENTLNG,ROTIX,DPHI,NOR,LEM,IBC,IBNGH,
     :                  IBCTYP,NCOMP,NUP,NE,NN,NBP,XPT0,YPT0,
     :                  POLEP,IBPOLE,TAPERF,IPOLE,IDEFTYP,
     :                  LBC,LUW,LSC,IDBUG,IERR)
C
C    This routine tests each point on the IBC table using the boundary
...
C
      DO 500 JX=1,NBP
        IF(IBCTYP(JX).LT.10)THEN     !don't hit fault nodes
          JY=JX+NBP
          NODE=IBC(JX)
          NNODE=NOR(NODE)
          XN=EX(NNODE)
...
C
     ...
     :         ((AZ2.LT.AZ1).AND.((AZA.GE.AZ1).OR.(AZA.LE.AZ2))))THEN
              DLEN=DLEN+DLENA
              XBIF=XBIF+XDIFA
              YBIF=YBIF+YDIFA
            ENDIF
            IF(((AZ2.GE.AZ1).AND.(AZB.GE.AZ1).AND.(AZB.LE.AZ2)).OR.
     :         ((AZ2.LT.AZ1).AND.((AZB.GE.AZ1).OR.(AZB.LE.AZ2))))THEN
              DLEN=DLEN+DLENB
              XBIF=XBIF+XDIFB
              YBIF=YBIF+YDIFB
            ENDIF
          END IF
        END IF ! if traction get segment length and orientation
C
...
C
        IF(ITAP.NE.0)THEN
          S1=AZ1
          S2=AZ2
          IF((AZ1.GE.AZ2).AND.(AZ.LE.AZ2))THEN
            S1=AZ1-TOOPI
          ELSE IF((AZ1.GE.AZ2).AND.(AZ.GE.AZ1))THEN
            S2=AZ2+TOOPI
          ENDIF
          IF(IPR.EQ.4)THEN         ! Cartesian
            CALL TAPER(ITAP,S1,S2,UTVAL1,UTVAL2,AZ,UTVAL)
          ELSE IF(IPR.EQ.5)THEN    ! spherical sheet
            CALL PTAPER(ITAP,S1,S2,AZ,CSRAT,
     :                  POLEP(1,IP1),POLEP(1,IP2),POLET)
        ...
        END IF !if ITAP
C
C   compute displacement rates derived from a pole of rotation
C   POLET(1,2,3) are long, lat and rate for pole
C
        IF(IPR.EQ.5)THEN
          IBPOLE(JX,1)=IP1
          IF(ITAP.EQ.0)IBPOLE(JX,2)=0
          IBCTYP(JX)=0
          IBCTYP(JY)=0
C
C    if spherical projection / coordinate rotation invoked
C    XY, YN rescaled to long, lat in degrees (and then discarded)
C    VELNE expects parameters in degrees; If rotation rate is 1
C    velocity at a distance of 90 degrees is 1.
C
          IF(IDEFTYP.GE.110)THEN
            PHI0=CENTLNG
            IF(IDEFTYP.EQ.111)PHI0=0.0
            XN=PHI0+XN/(DTOR*COS(YN-PION2))
            YN=(YN-PION2)/DTOR
          ENDIF
          CALL VELNE(POLET(2),POLET(1),YN,XN,UYV,UXV)  ! (lat, long)
          QBND(JX)=UXV*POLET(3)
          QBND(JY)=UYV*POLET(3)
        ELSE
C
...
C   in VSBCBE, boundary tractions are scaled by Argand number
C              this scaling facilitates search of parameter space
C   in VSBCON, boundary tractions are NOT scaled by Argand number
C
...
C****************************************************************************************
C
      SUBROUTINE VSBSET(QBND,EX,EY,NOR,IBC,IBCTYP,POLEP,IBPOLE,
     :                  TAPERF,CENTLNG,NCOMP,NBP,NUP,LUW,LSC)
C
...
C
          BPLAT=RTOD*EY(NNODE) - 90.0
          BPLON=CENTLNG+RTOD*(XN/SIN(YN))
C
C     apply the taper using the TAPERF value if required
C
          IP2=IBPOLE(JX,2)
          IF(IP2.NE.0)THEN
            TAPER=TAPERF(JX)
...
C         IBCTYP(JY)=0
          IF(POLET(3).NE.0.0)THEN
            ...
            QBND(JY)=UYV*POLET(3)
            NC=NC+1
C           IF((BPLON.GE.83.0).AND.(BPLON.LE.85.0))    ! for debugging
C    :      WRITE(*,*)'VSBSET: LON, LAT, VE, VN=',
C    :                BPLON,BPLAT,QBND(JX),QBND(JY)
