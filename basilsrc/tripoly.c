...
{
  char str[32], buf[81];
  int np,ns,natt,nbm,ndim,nh,nr,bndmark;
  int i,j,min;
  int rtnval=0;
  ...
  for (i=0;i<num;i++){
    ...
    /*
     * determine if the seg endpts are numbered 0...ns-1 or 1...ns
     */
  for (i=0,min=numseg;i<numseg;i++){
      if (segs[i*3]<min) min=segs[i*3];
      if (segs[i*3+1]<min) min=segs[i*3+1];
  }
  inp->numberofsegments = ns = numseg;
  if ((inp->segmentlist = (int *) malloc(
                 inp->numberofsegments * 2 * sizeof(int)))==0) {
      rtnval=fprintf(stderr,"Malloc failed in trimesh\n");
      return(1);
