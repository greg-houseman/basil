...
C
      EPS=1.E-5
C     WRITE(*,*)'Entered PERI , NFPF3,NFP=',NFPF3,NFP
      IF(NFP.LE.NFPF3)THEN
        WRITE(LUW,10099)
        WRITE(LSC,10099)
        RETURN
      ENDIF
Cle     DO K = NFPF3+1,NFP
Cle       JFBC1(K)=0
Cle       JFBC2(K)=0
Cle     ENDDO
C
...
C
      INDXF=NFPF3
      IF(IXY(1:1).EQ.'X')THEN        ! periodicity in the X-direction
        DO I=1,NBP
          II=IBC(I)
          IPRES=0
          IF(NFPF3.GT.0)THEN
...
C               WRITE(*,*)'Found node II, IPRES==',II,IPRES
   ...
   10     PA1=EX(NOR(II))
     ...
     :         (ABS(PB1-PB2)      .LT.EPS))THEN
          ...
          ENDDO                  ! loop on J
        ENDDO                    ! loop on I
      ELSE                       ! periodicity in the Y-direction
        DO I=1,NBP
          II=IBC(I)
          IPRES=0
          IF(NFPF3.GT.0)THEN
...
C               WRITE(*,*)'Found node II, IPRES==',II,IPRES
   ...
   20     PA1=EY(NOR(II))
     ...
     :         (ABS(PB1-PB2)      .LT.EPS))THEN
          ...
          ENDDO
        ENDDO
      ENDIF                        ! X-direction or Y-direction periodicity (IXY)
C     WRITE(*,*)'IFEQV array follows NFP=',NFP
C     CALL MITPP(IFEQV,NFP)
...
C
      ...
      DO JEL=1,NE
        ...
        ENDDO
      ENDDO
      WRITE(LUW,10100)IXY(1:1),PDIST,INDXF-NFPF3
      WRITE(LSC,10100)IXY(1:1),PDIST,INDXF-NFPF3
C
C    Finally, set IBCTYP = 11 as used for a locked fault (still necessary ?)
C
      DO K=NFPF3+1,NFP
        NIBC=IFBC2(K)
        IBCTYP(NIBC)=11
...
10100 FORMAT('Periodic boundaries in ',A1,'-direction now implemen',
     :'ted, lambda = ',F8.4,' with ',I5,' nodes set.')
      END
C
     ...
     :                   NSEG,LUW,LSC,IERR)
C
C   this routine is called by the MESH command if IMSH=3 and IFLT > 2 to 
...
C   IFBC1 = the node numbers that fall on a fault
C   IFBC2 = the position in the array IBC of the node given in IFBC1
C   IFEQV = gives the position in the array IFBC of the equivalent node 
C          on the other side of the fault (e.g. if J=IFEQV(I),
C          then IFBC(I) and IFBC(J) are across the fault from each other)
...
C             WRITE(*,*)'Fault node pairs are I,J,N1,N2 = ',I,J,N1,N2
      ...
      ENDDO
C
C   to associate element numbers and midpoints, search through the elements:
C   external boundary and faults: midpoint is present in one element
C   otherwise a midpoint element will be present in 2 elements
C
      DO K=1,NUP
        NTMP(K)=0
      ENDDO
      ...
      DO JEL=1,NE
        DO JM=4,6
          JMPN=LEM(JM,JEL)
          NTMP(JMPN)=NTMP(JMPN)+1
        ENDDO                    ! JM   - midpoint node
      ENDDO                      ! JEL  - element number
C
C    remove vertex nodes from list in NTMP.  Values of 1 that remain indicate
C    midpoint nodes on external boundary or internal fault
C
      NTOT=0
      DO K=1,NUP
        IF(NTMP(K).GE.2)NTMP(K)=0
        IF(NTMP(K).EQ.1)NTOT=NTOT+1
      ENDDO
C
C  now work through the elements again to assign values to JFBC arrays
C  for midpoint nodes that are found in IFBC and NTMP
C
      IFOUND=0
      DO JEL=1,NE
        DO JM=4,6
          JMPN=LEM(JM,JEL)
          IF(NTMP(JMPN).EQ.1)THEN
            DO K=1,NFP
...
C          node in IFBC1. If JFBC1=0 then JFBC2=0
C
C     WRITE(*,*)'SEMFLT, NBP, NFP, NFPF3 =',NBP,NFP,NFPF3
C     WRITE(*,*)'SEMFLT: IFBC1 follows'
C     CALL MITPRT(IFBC1,2,NFP/2,NFP)
C     WRITE(*,*)'SEMFLT: IFBC2 follows'
C     CALL MITPRT(IFBC2,2,NFP/2,NFP)
C
      NFPO2=NFP/2
      IERR=0
      DO I=1,NK2
   ...
   15     CONTINUE
          DO 20 JJ=1,3
            IF(LKK(JJ).NE.0) THEN
              ...
              IDST(LKK(JJ))=IDST(LKK(JJ))+1     ! counter to be removed
            END IF
   20     CONTINUE
        END IF
   10 CONTINUE
C     DO K=1,NFP
C       WRITE(*,*)'K,QNX(K),QNY(K),IDST(K)=',K,QNX(K),QNY(K),IDST(K)
C     ENDDO
C
...
C     CALL DMATPRT(STK2,2,NK2/2,NFP)
