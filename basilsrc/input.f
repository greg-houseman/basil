...
C
     ...
     :                       ITHDI,POLYFILE,IREAD,MAXNBS,JNO,KPBD,
...
C
   ...
   4          CONTINUE
              ...
              XLEN = RVARS(IXLEN)
              YLEN = RVARS(IYLEN)
              AREA = RVARS(IELAREA)     ! 3 lines added, GH, 4/5/15
...
C
     ...
     :               UTVAL1,UTVAL2,ITAP,IPR,XMIN,XMAX,YMIN,YMAX,IERR)
            IF(IXY(1:1).EQ.'X')KPBD=1
            IF(IXY(1:1).EQ.'Y')KPBD=2
...
C
      ...
      IF((IFLT.EQ.0).AND.(KPBD.GT.0))IFLT=1
      IF (ISET.NE.1) THEN
...
C               NOR(K)=K
     ...
     :                                     LUW,LSC,IERROR)
           IF (IERROR.NE.0)THEN
            ...
            ENDIF
C
C          Establish the tables which identify boundary nodes
C          and connections
C
            IF (IV(IIMSH).NE.3) THEN
              CALL VSBTAB(RV(IXLEN),RV(IYLEN),EX,EY,LEM,NOR,IBC,IBNGH,
     :             XZERO,YZERO,IBCTYP,KORNER,IV(INUP),IV(INE),IV(INBP),
     :                  IV(IIFLT),LUW,LSC,IERROR)
              IF (IERROR.NE.0)THEN
                 IERROR=25
                GO TO 700
              ENDIF
C  HUGE problem for IIMSH=3 - is it le or gh??????? [need more info...greg]
C  It is ok to let VSBTAB2 redo bnd arrays UNLESS trimesh
C  has set up faults
            ELSE IF (IV(IIFLT).EQ.0) THEN
...
C
     ...
     :                  EX,EY,QBND,POLEP,IPOLE,IBPOLE,TAPERF,
